...
def percentile_safe(values: pd.Series, q: float) -> float:
    """Return percentile q for non-NaN values, or NaN if the series is empty."""
...
def calibrate_piecewise_breaks(
    ...
    # If fewer than 2 breaks, fall back to a global range
    if len(breaks) < 2:
...
def plot_sigma_and_speed_by_scenario(
    ...
    output_dir: str = "data/piecewise_plots/piecewise_model_scenarios",
) -> None:
    """
    ...
    Each take (label) is plotted as a separate line, with time shifted
    so that each take starts at t = 0. Output is a JPG in output_dir.
    """
    sigma_col = f"sigma_{axis}"
    ...
    out_path = os.path.join(output_dir, f"{scenario}_{axis}_piecewise.jpg")
    ...
    ax_sigma.set_title(f"{scenario} – {axis}: σ and InterpSpeed (piecewise)")

    ax_sigma.legend(fontsize="small", ncol=2)
    plt.tight_layout()
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[piecewise_sigma_model] Saved scenario σ/speed plot to {out_path}")
...
def plot_sigma_bar_by_scenario(
    ...
    One figure per axis, one bar per scenario. Uses the same scenario
    order as the linear model for direct comparison.
    """
    sigma_col = f"sigma_{axis}"
    ...
    plt.ylabel(r"$\sigma$")
    plt.xlabel("scenario")
    plt.title(rf"$\sigma$ (90th percentile) by scenario – {axis} (piecewise)")
    ...
    print(f"[piecewise_sigma_model] Saved σ bar plot to {out_path}")
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Output CSV with sigma_* and InterpSpeed_* "
            "(default: data/modeled/tracking_modelled_sigma_piecewise.csv)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Scenario to plot σ and InterpSpeed for (overlays all takes) – "
            "used with --plot-axis"
        ),
    )
    parser.add_argument(
        "--plot-axis",
        type=str,
        help="Axis to plot for scenario plots (e.g. X_pose, Y_rot).",
    )
    parser.add_argument(
        "--plot-bar-axis",
        type=str,
        help=(
            "Axis to plot σ bar chart for across all scenarios " "(e.g. X_pose, Y_rot)."
        ),
    )
    ...
    print("[piecewise_sigma_model] Created CSV written to", args.output)
    ...
    ensure_dir_for_file(args.config_output)
    with open(args.config_output, "w", encoding="utf-8") as f:
        json.dump(cfg, f, indent=2)
    print("[piecewise_sigma_model] σ breaks and speeds saved to", args.config_output)
