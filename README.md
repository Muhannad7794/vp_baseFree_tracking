...
### 3.2 Basic pipeline (rebuild all from processed data)
...
The default command (as set in `docker-compose.yml`) will typically execute a small driver script or a sequence like:
...
> **Note:** For the image paths below to work in GitHub or other viewers, the corresponding JPG files must be committed to the repository in those locations. They can all be generated by the commands given.
